...
Metropolitan France is divided into 13 regions, 96 departments, and
...
Spectral Clustering \cite{luxburg_tutorial_2007} helps us overcome two major
...
\begin{figure}[!h]
    ...
    \begin{tikzpicture}[
        ...
        %Nodes
        \node[roundnode]      (hospital1)     [xshift=-0.5cm]                       {Hospital 1};
        \node[roundnode]      (hospital2)     [right=of hospital1, xshift=0.5cm]    {Hospital 2};
        \node[squarednode]    (patient)       [below=of hospital1, xshift=2cm]    {Patient};

        %Lines
        \draw[<->] (hospital1.east) -- node[anchor=south] {co-occ} (hospital2.west);
        \draw[->]  (patient.north) -- (hospital1.south);
        \draw[->]  (patient.north) -- (hospital2.south);
    \end{tikzpicture}
    \caption{
...
\begin{figure}[!h]
    ...
    \begin{tikzpicture}[
        ...
        %Nodes
        \node[bluenode]      (1)                       {1};
        \node[bluenode]      (2)   [below=of 1]        {2};
        \node[bluenode]      (3)   [right=of 1]        {3};

        \node[rednode]       (4)   [right=of 3]        {4};
        \node[rednode]       (5)   [right=of 4]        {5};
        \node[rednode]       (6)   [below=of 5]        {6};
        ...
        \draw[-] (4.east) -- (5.west);
        \draw[-] (4.south east) -- (6.north west);
        \draw[-] (5.south) -- (6.north);

    \end{tikzpicture}
    \caption{
        \textbf{Co-occurrence graph between the hospitals.} Hospitals
        are represented as nodes, and edges are weighted by the number of
        co-occurrences between them. On this graph, there are two communities
        colored in blue and red that we would like to retrieve.
    }
    \label{diagram:co-occ-graph}
\end{figure}

Community detection algorithms are used to evaluate how groups of nodes are
...
\subsection{Oncology specialization label}
...
centers from clusters 1 and 3. The activity of the 79 hospitals in the cluster 1
combined equals 36.8\% of the total activity. This is almost as large as the
activity of the n=451 hospitals from cluster 3. As mentioned in the
centralization of care benefits, the care centers from the cluster 1 probably
have the largest expertise due to the large volume of cancer patients they
treat.
