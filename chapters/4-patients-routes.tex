...
weighted by $\sigma$. The lower $\sigma$ is, the closer we get to the
...
of the algorithm is the optimal transport plan $P^*$, that moves the input
...
that the hospital $j$ can host. The $P^*$ output matrix contains the
overall proportions of patients sent from the municipality $i$ to the
hospital $j$, so that $\sum_{i,j} P_{ij} = 1$.
...
\begin{figure}[h!]
    ...
    \label{fig:travel-burden-alluvium}
\end{figure}
