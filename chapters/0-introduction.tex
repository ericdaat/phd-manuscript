...
\begin{itemize}
    ...
    \item \textbf{Immunotherapy.} Immunotherapy is a treatment that that helps
          the immune system fight cancer. % TODO: write more
...
% INCa & cancer plans
In France, The National Cancer Institute (INCa) is the State agency for health
...
Setting up actions aimed at reducing inequalities in the face of cancer, since
...
inequalities.
The Cancer Plan also provides for increased support for interventional
...
\subsection{The rise of Machine Learning applied to Healthcare}

% TODO: re-write

Artificial Intelligence techniques, most of which were conceived in the last
century, have been booming in recent years, driven by technological advances and
the systematization of data collection. The use of these techniques is becoming
widespread and democratized in many sectors such as robotics, digital companies
and more recently in medicine. Neural networks" are for example used in medicine
to predict the appearance of future diseases or to interpret medical images. The
development of predictive models using neural networks would allow to improve
and personalize the care pathway of patients, in particular by alerting them to
the risks of relapse or second cancer. Previous works have used care pathways to
create predictive models. For example, the work of Lee and Ho [21] developed a
pattern detection algorithm in care pathways to predict the risk of heart
failure. Haas et al [22] developed a treatment recommendation model based on
genetic, clinical and demographic information for patients with non-small cell
lung cancer. Zhang et al [23] developed Patient2Vec, an algorithm capable of
learning care pathway representations to predict re-hospitalization. Finally, Wu
et al [24] developed a survival and relapse prediction model for breast cancer
patients using clinical data enriched with external data such as smoking status.

\subsection{Leveraging medical data in France}

% TODO: re-write

Real-life data represents an unprecedented and currently underutilized volume of information.
currently under-exploited. In particular, in France, the Social Security
generates a large structured database for administrative purposes: the
National Health Data System (SNDS). The SNDS gathers complete and up-to-date
and up to date administrative data on 98\% of the French population (anonymous
population (anonymous identifier, age, sex, affiliation regime, department of
residence, long-term conditions (ALDs)), reimbursement data for health care expenses
reimbursement data (procedures, consultations, drugs, medical devices) and
(DCIR)) and hospital data (stays, diagnoses, procedures,
PMSI external activity). The exploitation of the SNDS, via the Plateforme de
for research purposes is an exceptional opportunity to broaden the scope of research
opportunity to broaden the scope of research in the improvement of the patient
patient pathway. Indeed, the substantial number of patients it contains exceeds
the size of all the French cohorts collected in the treatment centers, a major limitation of which
treatment centers, a major limitation of which is the insufficient updating of
medical records (lack of long-term follow-up, lack of communication between
